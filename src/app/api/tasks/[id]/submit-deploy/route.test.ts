...
function makeRequest(body: Record<string, unknown>) {
  return new NextRequest('http://localhost/api/tasks/1/submit-deploy', {
    method: 'POST',
...
describe('POST /api/tasks/[id]/submit-deploy', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    mockRequireRole.mockResolvedValue({ valid: true, role: 'devops', keyId: 4, name: 'devops-key' });
  ...
  it('moves to done on success', async () => {
    mockLimit.mockResolvedValueOnce([{ id: 1, status: 'deploying' }]);
    mockReturning.mockResolvedValueOnce([{ id: 1, status: 'done' }]);
    mockValues.mockResolvedValueOnce(undefined);

    const res = await POST(makeRequest({ result: 'success', url: 'https://prod.example.com' }), { params: Promise.resolve({ id: '1' }) });
    expect(res.status).toBe(200);
    const json = await res.json();
    expect(json.data.status).toBe('done');
  });

  it('moves to in-progress on failure', async () => {
    mockLimit.mockResolvedValueOnce([{ id: 1, status: 'deploying' }]);
    mockReturning.mockResolvedValueOnce([{ id: 1, status: 'in-progress' }]);
    mockValues.mockResolvedValueOnce(undefined);

    const res = await POST(makeRequest({ result: 'failure', notes: 'Build failed' }), { params: Promise.resolve({ id: '1' }) });
    expect(res.status).toBe(200);
    const json = await res.json();
    expect(json.data.status).toBe('in-progress');
  });

  it('calls requireRole with devops and admin', async () => {
    mockLimit.mockResolvedValueOnce([{ id: 1, status: 'deploying' }]);
    mockReturning.mockResolvedValueOnce([{ id: 1, status: 'done' }]);
    mockValues.mockResolvedValueOnce(undefined);

    await POST(makeRequest({ result: 'success' }), { params: Promise.resolve({ id: '1' }) });
    expect(mockRequireRole).toHaveBeenCalledWith(expect.anything(), ['devops', 'admin']);
  });
});
