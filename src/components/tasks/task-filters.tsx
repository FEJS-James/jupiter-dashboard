    ...
    <Card className="bg-slate-800/50 border-slate-700">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          {activeFilterCount > 0 && (
            <Badge className="bg-blue-600 text-white text-[10px] px-1.5 py-0.5 h-5">
          ...
          {/* Inline active filter badges when collapsed */}
          {!expanded && hasActiveFilters && (
            <div className="flex items-center gap-1.5 ml-1">
              {filters.statuses.length > 0 && (
                <Badge variant="secondary" className="bg-green-900/30 text-green-300 border-green-700 text-[10px] h-5 px-1.5">
                  <Clock className="w-3 h-3 mr-1" />
                  {filters.statuses.length}
                </Badge>
              )}
              {filters.priorities.length > 0 && (
                <Badge variant="secondary" className="bg-yellow-900/30 text-yellow-300 border-yellow-700 text-[10px] h-5 px-1.5">
                  <AlertCircle className="w-3 h-3 mr-1" />
                  {filters.priorities.length}
                </Badge>
              )}
              {filters.assignees.length > 0 && (
                <Badge variant="secondary" className="bg-purple-900/30 text-purple-300 border-purple-700 text-[10px] h-5 px-1.5">
                  <Users className="w-3 h-3 mr-1" />
                  {filters.assignees.length}
                </Badge>
              )}
              {filters.search && (
                <Badge variant="secondary" className="bg-blue-900/30 text-blue-300 border-blue-700 text-[10px] h-5 px-1.5">
                  <Search className="w-3 h-3 mr-1" />
                  &quot;{filters.search.length > 12 ? filters.search.slice(0, 12) + '…' : filters.search}&quot;
                </Badge>
              )}
            </div>
          )}
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs text-slate-400 flex items-center gap-1.5">
            ...
            {filterStats.filteredTasks}/{filterStats.totalTasks}
          </span>
          {hasActiveFilters && (
            <Button
              variant="ghost"
              size="sm"
              className="text-slate-400 hover:text-slate-200 h-6 text-xs px-2"
